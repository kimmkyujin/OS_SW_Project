...
```
이 다이어그램은 해당 앱이 어떻게 작동하고 화면이 어떻게 넘어가는지를 한눈에 보여주는 설계도이며 처음 앱을 실행하면 로그인 화면이 나오고 회원가입을 거쳐 로그인을 하면 시스템이 일반 사용자인지 관리자인지를 확인해서 각각의 전용 화면으로 보내준다. 일반 사용자는 홈 화면을 중심으로 보관소나 추천 메뉴 그리고 문의사항 페이지를 자유롭게 이동할 수 있는데 다른 화면에서 뒤로가기를 누르면 무조건 홈 화면으로 돌아오게 만들어서 사용자가 길을 잃지 않게 설계했다. 특히 레시피 상세 화면에서는 사용자가 도구나 인원수를 바꿀 때마다 조리법 문장이 자동으로 변하는 똑똑한 변환 과정을 거치게 되며 문의사항 화면에서는 사용자가 글을 쓰고 나중에 관리자가 답변을 달면 상태가 완료로 바뀌는 과정을 보여준다. 마지막으로 앱을 완전히 종료하고 싶을 때는 오직 홈 화면에서만 종료 확인창이 뜨게 만들었으며 여기서 예를 누르면 로그인 정보가 지워지면서 앱이 안전하게 꺼지기 때문에 다음번에 실행할 때 다시 로그인부터 시작하도록 보안까지 신경써주었다.
